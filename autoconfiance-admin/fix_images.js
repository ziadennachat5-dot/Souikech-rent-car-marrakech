...
try {
    ...
    let currentCategory = 'citadine'; // default fallthrough
    ...
    const newLines = lines.map(line => {
        ...
        if (line.trim().startsWith('image:')) {
            // If it looks like a local path (c: or /) or is malformed, replace it.
            // We reserve https:// links if they seemingly don't look like the broken local ones.
            // The broken ones might be 'c:\\Users' or 'c:Users' or '/peugeot...'.
            // Valid ones are 'https://...'

            const isUrl = line.includes('https://') || line.includes('http://');
            const isLocal = line.includes('c:') || line.includes('C:') || line.includes('Users');

            if (!isUrl || isLocal) {
                console.log(`Fixing: ${line.trim()} -> ${currentCategory}`);
